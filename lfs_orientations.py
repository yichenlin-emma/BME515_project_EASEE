...
for a in angles: ## iterate through different orientations
    ...
    def update_field():
        phi_e = []
        for node_ind, node in enumerate(nodes):
            if count < 3:  # this is for the first possible position and its rotations
                x_loc_l = 1e-3 * (
                            -(n_nodes - 1) / 2 * inl + inl * node_ind - 15)  # 1e-3 [um] -> [mm], for left electrodes
                x_loc_r = 1e-3 * (
                            -(n_nodes - 1) / 2 * inl + inl * node_ind + 15)  # 1e-3 [um] -> [mm], for right electrodes
                ...
                # radii
                r_l = np.sqrt(x_loc_l ** 2 + 15 ** 2 + e2f_1 ** 2 - 2 * x_loc_l * dist * np.cos(
                    np.radians(a)))  # [mm], same for both
                # front and back left electrodes
                r_r = np.sqrt(x_loc_r ** 2 + 15 ** 2 + e2f_1 ** 2 - 2 * x_loc_r * dist * np.cos(
                    np.radians(a)))  # [mm], same for both
                # front and back right electrodes
            ...
            else:  # this is for the second one
                x_loc_l = 1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind - np.sqrt(
                    2 * 15 ** 2))  # 1e-3 [um] -> [mm], for left center electrode
                x_loc_r = 1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind + np.sqrt(
                    2 * 15 ** 2))  # 1e-3 [um] -> [mm], for right center electrode
                x_loc_c = 1e-3 * (-(
                            n_nodes - 1) / 2 * inl + inl * node_ind)  # 1e-3 [um] -> [mm], for top, bottom and central center electrode

                # y distance is sqrt(2*(15mm)^2) for top and bottom electrodes, and 0mm for central ones
                dist = np.sqrt(15 ** 2 + e2f_1 ** 2)
                # radii
                r_c_l = np.sqrt(
                    x_loc_l ** 2 + e2f_1 ** 2 - 2 * x_loc_l * e2f_1 * np.cos(np.radians(a)))  # [mm], for left central
                r_c_r = np.sqrt(
                    x_loc_r ** 2 + e2f_1 ** 2 - 2 * x_loc_r * e2f_1 * np.cos(np.radians(a)))  # [mm], for right central
                r_c_c = np.sqrt(
                    x_loc_c ** 2 + e2f_1 ** 2 - 2 * x_loc_c * e2f_1 * np.cos(
                        np.radians(a)))  # [mm], for central electrode
                r_t_b = np.sqrt(
                    x_loc_c ** 2 + np.sqrt(2 * 15 ** 2) + e2f_1 ** 2 - 2 * x_loc_c * e2f_1 * np.cos(
                        np.radians(a)))  # [mm], for top and bottom electrode

                # e-field for 5 electrodes positioned around fiber
                node(0.5).e_extracellular = (electrode.i) / (4 * sigma_e * np.pi * r_c_l) + (electrode.i) / (
                        4 * sigma_e * np.pi * r_c_r) + (electrode.i) / (4 * sigma_e * np.pi * r_c_c) + (
                                                        2 * electrode.i) / (4 * sigma_e * np.pi * r_t_b)
