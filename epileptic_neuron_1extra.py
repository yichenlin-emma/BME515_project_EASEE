...
dummy_1 = h.Section(name='dummy_1')
electrode_1 = h.IClamp(dummy_1(0.5)) # puts the stim halfway along the length
electrode_1.delay = e1_delay
electrode_1.dur = e1_dur
e1_stim_wave_vec.play(electrode_1._ref_amp, time_vec, True)  # setting extracell. stimulation to rectangular wave
...
def update_field():
    ...
    for node_ind, node in enumerate(nodes):
        ...
        r = np.sqrt(x_loc**2 + e2f_1**2) # [mm]
        node(0.5).e_extracellular = (electrode_1.i)/(4*sigma_e*np.pi*r)
...
ax2.set(xlabel="time (ms)", ylabel="intracell. stimulus voltage (mV)")
ax3.plot(t[:10000], e1_stim_wave[:10000])
ax3.set(xlabel="time (ms)", ylabel="extracell. stimulus voltage (mV)")
plt.tight_layout()
plt.show()
