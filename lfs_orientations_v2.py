...
# stimulus parameters
# look at different fiber orientations
# biggest firing rate reduction for LFS at 130Hz epilepsy --> so let's use this frequency
...
angles = [0, 45, 90, 0, 45]  # [°] angle of orientation
positions = [1,2,3,4,5]  # different positions according to model
...
epilep_amp = 2  # 0.2 # [mA]: amplitude of (intracellular) stim object (negative cathodic, positive anodic), chosen amplitude
# that def. triggers AP, determined via experiments
# we want to have a rectangular stimulation pattern of a certain frequency:
epilep_f = 130*(10**-3)# [Hz * 10**-3 = ms^-1]: frequency of intracell. stim, firing rates above 100Hz
...
count = 1
for a in angles: ## iterate through different orientations
    ...
    def update_field():
        phi_e = []
        for node_ind, node in enumerate(nodes):
            if count == 1:  # this is for position 1
                ...
                # y distance distance to electrode outside dkull
                y_dist = e2f  # for all
                ...
                # radii
                r_l = np.sqrt(x_loc_l ** 2 + y_dist ** 2 + z_lr**2)  # [mm], for left electrodes
                r_r = np.sqrt(x_loc_r ** 2 + y_dist ** 2 + z_lr **2)  # [mm], for right electrodes
                r_c = np.sqrt(x_loc_c ** 2 + y_dist ** 2 + z_c **2)  # [mm], for right electrodes

                # e-field for 5 electrodes positioned around fiber
                node(0.5).e_extracellular = (2 * electrode.i) / (4 * sigma_e * np.pi * r_l) + (2 * electrode.i) / (
                            4 * sigma_e * np.pi * r_r) + \
                                            (electrode.i) / (4 * sigma_e * np.pi * r_c)

            elif 1 < count < 4:  # this is for position 2 & 3 (rotate in y direction)
                x_loc_l = (1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind) - 15) * np.cos(a)  # 1e-3 [um] -> [mm], for left electrodes
                x_loc_r = (1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind) + 15) * np.cos(a)  # 1e-3 [um] -> [mm], for right electrodes
                x_loc_c = (1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind)) * np.cos(a)  # 1e-3 [um] -> [mm], for center electrode

                # y distance distance to electrode outside dkull
                y_l = e2f + np.tan(a)*x_loc_l
                y_r = e2f - np.tan(a) * x_loc_r
                y_c = e2f
                ...
                # radii
                r_l = np.sqrt(x_loc_l ** 2 + y_l ** 2 + z_lr ** 2)  # [mm], for left electrodes
                r_r = np.sqrt(x_loc_r ** 2 + y_r ** 2 + z_lr ** 2)  # [mm], for right electrodes
                r_c = np.sqrt(x_loc_c ** 2 + y_c ** 2 + z_c ** 2)  # [mm], for right electrodes
            ...
            elif count == 4:  # this is for position 4 (rotate in z direction only)
                # now have 3 "center" electrodes, 1 left, 1 right
                x_loc_l = (1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind) - 15) # 1e-3 [um] -> [mm], for left electrodes
                x_loc_r = (1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind) + 15) # 1e-3 [um] -> [mm], for right electrodes
                x_loc_c = (1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind)) # 1e-3 [um] -> [mm], for center electrode

                # y distance distance to electrode outside skull
                y_dist = e2f
                ...
                # radii
                r_cc = np.sqrt(x_loc_c ** 2 + y_dist ** 2 + z_clr ** 2)  # [mm], for left electrodes
                r_cl = np.sqrt(x_loc_l ** 2 + y_dist ** 2 + z_clr ** 2)  # [mm], for right electrodes
                r_cr = np.sqrt(x_loc_r ** 2 + y_dist ** 2 + z_clr ** 2)  # [mm], for right electrodes
                r_tb = np.sqrt(x_loc_c ** 2 + y_dist ** 2 + z_c_tb ** 2)  # [mm], for right electrodes

                # e-field for 5 electrodes positioned around fiber
                node(0.5).e_extracellular = 2 * electrode.i/(4 * sigma_e * np.pi * r_tb) \
                                            + electrode.i/(4 * sigma_e * np.pi * r_cc) \
                                            + electrode.i/(4 * sigma_e * np.pi * r_cl) \
                                            + electrode.i/(4 * sigma_e * np.pi * r_cr)

            elif count == 5:  # this is for position 5 (rotation in z and y direction)
                x_loc_l = (1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind) - 15) * np.cos(a)  # 1e-3 [um] -> [mm], for left electrodes
                x_loc_r = (1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind) + 15) * np.cos(a)  # 1e-3 [um] -> [mm], for right electrodes
                x_loc_c = (1e-3 * (-(n_nodes - 1) / 2 * inl + inl * node_ind))  # 1e-3 [um] -> [mm], for center electrode
                ...
                # radii
                r_cc = np.sqrt(x_loc_c ** 2 + y_c ** 2 + z_clr ** 2)  # [mm], for left electrodes
                r_cl = np.sqrt(x_loc_l ** 2 + y_l ** 2 + z_clr ** 2)  # [mm], for right electrodes
                r_cr = np.sqrt(x_loc_r ** 2 + y_r ** 2 + z_clr ** 2)  # [mm], for right electrodes
                r_tb = np.sqrt(x_loc_c ** 2 + y_c ** 2 + z_c_tb ** 2)  # [mm], for right electrodes
    ...
    firing_rates_before.append(firing_rate_before)
    firing_rates_after.append(firing_rate_after)
...
ax.plot(positions, firing_rates_before, label="before stimulus")
ax.plot(positions, firing_rates_after, label="after stimulus")
ax.set(xlabel="Position according to Graph", ylabel="Firing Rate (Hz) ",
        title="Change of firing rate with extracellular \nLFS stimulus according to fiber orientation")
